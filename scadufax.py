...
def flag_scan_single(single_target_address):
    try:
            ...
            for port_addr in range(0, len(destination_port_address)):
                ...
                if (packet_status != None):
                    ...
                    if (packet_status[1].strip() == 'open'):
                        ...
                        pass
        elif (icmp_packet == False):
            printf_hostdown(single_target_address)

            # if output enabled, print it into a file
            if (args.o != None):
                output_logging(args.o, '[offline] ' + single_target_address + '\n')
        ...
        print ('\nQuiting, program terminated by user.')
...
# icmp scanner
def icmp_scan(scan_from, scan_to):
    try:
        scadufax_banner()
        ...
        # load profile configuration
        load_scadufax_profile(scan_from + '-' + scan_to, 'None', 'None', request_timeout)
        ...
        if (args.o != None):
            output_logging(args.o, '[*] scanning profile\n')
            output_logging(args.o, '\t[>] target address: ' + scan_from + '-' + scan_to + '\n')
            output_logging(args.o, '\t[>] source port: ' + 'None' + '\n')
            output_logging(args.o, '\t[>] destination port: ' + 'None   ' + '\n')
            output_logging(args.o, '\t[>] request timeout: ' + str(request_timeout) + '\n\n')
        ...
        for ip_addr in range(0, len(range_target_address)):
            ...
            target_address = str(range_target_address[ip_addr])
            try:
                icmp_request = icmp_send_packet(target_address, request_timeout)
                if (icmp_request == True):
                    printf_target(str(range_target_address[ip_addr]))
                    # print target address 
                    if (args.o != None):
                        output_logging(args.o, '\n[!] ' + str(range_target_address[ip_addr]))
...
def main():
    ...
    parser.add_argument('-t', dest='tcp_scanner', action='store_true',
                        help='TCP scanning')
    parser.add_argument('-icmp', dest='icmp_scanner', action='store_true',
                        help='ICMP Net Discover [root required]')
    ...
    if not len(sys.argv[1:]):
        ...
        if (args.flag_scanner != None and args.tcp_scanner == False and args.icmp_scanner == False):
            if args.flag_scanner in scan_type:
                ...
                if (args.d == None and args.R == None):
                    ...
                    # everything its ok, call the first layer
                    first_layer()
        elif (args.flag_scanner == None and args.tcp_scanner == False and args.icmp_scanner == True):
            if (args.d == None and args.R == None):
                # no target selected (single of multiples)
                program_usage('no target selected')
            elif (args.d != None):
                # user tried to execute -icmp option with -d (target only)
                program_usage('-icmp works with -R only')                            
            else:
                # everything its ok, call the first layer
                first_layer()

        elif (args.flag_scanner != None and args.tcp_scanner == True and args.icmp_scanner == True):
